...
def physical_scene_identity(scene: dict, mapping: str) -> str:
    """Count physical configurations independently of decorative random-seed labels."""
    return checksum({"scene": {k: v for k, v in scene.items() if k != "seed"}, "mapping": mapping})


def archive_sources(directory: Path) -> tuple[str, dict]:
    """Retain the exact execution source without overwriting a resumed campaign's protocol."""
    paths = (
        Path(__file__).resolve(),
        Path("crazyflow/safety/da_plcbf/navigation_experiment.py").resolve(),
        Path("crazyflow/safety/da_plcbf/case_study_world.py").resolve(),
    )
    data = {str(p.relative_to(Path.cwd())): p.read_bytes() for p in paths}
    hashes = {name: hashlib.sha256(value).hexdigest() for name, value in data.items()}
    identifier = checksum(hashes)
    source_root = directory / "source_versions" / identifier
    for name, value in data.items():
        target = source_root / name
        target.parent.mkdir(parents=True, exist_ok=True)
        if target.exists() and target.read_bytes() != value:
            raise ValueError("immutable source archive mismatch")
        target.write_bytes(value)
    write_json(source_root / "SOURCE.json", hashes)
    return identifier, hashes


def preserve_partial_attempt(directory: Path) -> Path | None:
    """Keep interrupted, unledgered pair files separate before replaying the complete pair."""
    if not directory.exists():
        return None
    number = 1
    while directory.with_name(f"{directory.name}.partial-{number}").exists():
        number += 1
    target = directory.with_name(f"{directory.name}.partial-{number}")
...
def classify_pair(methods: dict) -> dict:
    ...
    candidate = (
        f == "collision"
        and a == "separated"
        and not fixed.get("censored", False)
        and not adaptive.get("censored", False)
        and fixed["collider_upper_m"] < -0.002
    ...
    penalty += 2 * (f in {"censored", "unresolved"} or a in {"censored", "unresolved"})
    return {
...
class EpisodeEvaluator:
    ...
    def resources(self, mapping: str, world: Any) -> tuple:
        ...
        if (
            ...
            raise ValueError("discovery must preserve checkpoint prediction and command cadence")
    ...
    def run_method(
        ...
        *,
        ...
        for boundary in (freeze_at, revert_at):
            if boundary is not None and (
                not np.isfinite(boundary)
                or boundary < 0
                or not np.isclose(
                    boundary / scene.world_config().control_period,
                    round(boundary / scene.world_config().control_period),
                )
            ):
                raise ValueError("interventions must use a finite nonnegative control boundary")
        ...
        for index, when in enumerate(times):
            ...
            reverted = revert_at is not None and when >= revert_at - 1e-10
            used_snapshot = bundle.state if reverted else persistent
            ...
            decision = jax.block_until_ready(
                controller(state, used_snapshot.params, model, prediction, previous, goal_device)
            )
            controller_seconds = time.perf_counter() - started
            rows["time"].append(float(when))
            rows["state"].append(np.asarray(state))
            rows["version_used"].append(int(used_snapshot.library_version))
            rows["parameter_reverted"].append(reverted)
        ...
        summary = {
            ...
            "parameter_reversion_time_seconds": revert_at,
...
def main() -> None:
    ...
    source_id, source_hashes = archive_sources(args.output)
    current_protocol = {
    ...
    if not (args.output / "protocol.json").exists():
        write_json(args.output / "protocol.json", current_protocol)
    with (args.output / "executions.jsonl").open("a") as stream:
        stream.write(json.dumps(current_protocol, allow_nan=False) + "\n")
    engine = EpisodeEvaluator(jax.devices(args.device)[0])
    outcomes = Counter(r.get("outcome_class", "invalid_scene") for r in prior)
    distinct = {physical_scene_identity(r["scene"], r["mapping"]) for r in prior if "methods" in r}
    for index, proposal in enumerate(proposals):
        ...
        identity = physical_scene_identity(proposal["scene"], proposal["mapping"])
        record = {
            **proposal,
            "trial_id": trial_id,
            "seed": args.seed,
            "index": index,
            "physical_scene_identity": identity,
            "source_execution_id": source_id,
        }
        try:
            ...
            partial = preserve_partial_attempt(args.output / trial_id)
            if partial is not None:
                record["preserved_partial_attempt"] = str(partial)
    ...
    write_json(
        args.output / "COUNTS.json",
        {
            ...
            "distinct_full_pairs": len(distinct),
        },
    )
