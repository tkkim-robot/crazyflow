...
class HoverEncounterConfig:
    """A prescribed wind encounter with optional guards, movers, and an early waypoint leg.

    The original hover-then-navigate protocol remains the default. Obstacle indices are the
    primary ``incoming`` sphere, then ``additional_incoming``, then the static ``guards``.
    ``initial_velocity`` is expressed in world coordinates and applies only when the builder
    does not receive an explicit branch state.
    """
    ...
    additional_incoming: tuple[IncomingSphere, ...] = ()
    ...
    def from_dict(cls, data: dict[str, Any]) -> HoverEncounterConfig:
        ...
        def sphere_from_dict(value: dict[str, Any]) -> IncomingSphere:
            incoming = dict(value)
            for key in ("direction", "crossing_offset"):
                if key in incoming:
                    incoming[key] = tuple(incoming[key])
            return IncomingSphere(**incoming)

        if "incoming" in values:
            values["incoming"] = sphere_from_dict(values["incoming"])
        if "additional_incoming" in values:
            values["additional_incoming"] = tuple(
                sphere_from_dict(value) for value in values["additional_incoming"]
            )
        if "guards" in values:
            values["guards"] = tuple(
    ...
    def world_config(self) -> NavigationWorldConfig:
        ...
        config = NavigationWorldConfig(
            seed=self.seed,
            obstacle_count=1 + len(self.additional_incoming) + len(self.guards),
            dt=self.dt,
        ...
        config.validate()
        for sphere in (self.incoming, *self.additional_incoming):
            sphere.validate(self.duration_seconds)
        ...
        initial_velocity = _vector(self.initial_velocity, "initial velocity")
        if np.linalg.norm(initial_velocity) > config.speed_max:
            raise ValueError("initial velocity must satisfy the configured speed limit")
        if (
            not math.isfinite(self.navigation_start_seconds)
            or not 0 <= self.navigation_start_seconds < self.duration_seconds
            or not math.isclose(
            ...
            raise ValueError("navigation start must be a control boundary inside the episode")
...
def build_hover_encounter_world(
    ...
    *,
    ...
    state = (
        np.asarray((*hover, 0, 0, 0, 1, *config.initial_velocity, 0, 0, 0), dtype=float)
    ...
    movers = (config.incoming, *config.additional_incoming)
    count = world_config.obstacle_count
    means = np.asarray(
        [hover + np.asarray(sphere.crossing_offset) for sphere in movers]
    ...
    for index, sphere in enumerate(movers):
        direction = np.asarray(sphere.direction) / np.linalg.norm(sphere.direction)
        omega = sphere.speed_m_s / sphere.amplitude_m
        amplitudes[index] = direction * sphere.amplitude_m
        frequencies[index], phases[index] = omega, -omega * sphere.arrival_time_seconds
    radii = np.asarray(
        [sphere.radius_m for sphere in movers] + [guard.radius_m for guard in config.guards]
    )
    arrays = (
        state,
