...
def compose(output: Path, preflight: Path, case: Path) -> Path:
    ...
    labels = {
        ...
        "calm": "Matched fallback policies without explicit wind feedforward.",
        ...
        "restart": (
            "Next: separate case | original state and library restored; "
            "preflight learning is not carried over"
        ),
    }
    ...
    listing.write_text("".join(f"file '{path}'\n" for path in (output / "preflight.mp4", case)))
    ...
    report = {
        ...
        "timeline": [
            ...
            {
                "video_seconds": [19, 19 + len(expected_case) / FPS],
        ...
        "case_start_frame": case_start,
        "transition_pause_frames": 0,
        "restart_disclosure_seconds": [16, 19],
        "comparison_scope": labels["calm"],
