...
def load_presentation(
    ...
    pre_indices = (
        source_ids[_nearest_indices(source_times[available], before)]
        if before.size
...
def _compose(
    ...
    is_impact = presentation.metadata["trigger"]["obstacle_index"] is not None
    title = (
...
def render(args: argparse.Namespace) -> Path:
    ...
    try:
        ...
        for index, state in enumerate(presentation.states):
            ...
            lookat = state[:3].copy()
            lookat[2] = max(0.35, 0.72 * lookat[2])
    ...
    metadata = {
        ...
        "renderer_sha256": _sha256(source),
        "common_renderer_sha256": _sha256(
            Path(__file__).resolve().parents[2]
            / "crazyflow/safety/da_plcbf/mujoco_comparison_video.py"
        ),
