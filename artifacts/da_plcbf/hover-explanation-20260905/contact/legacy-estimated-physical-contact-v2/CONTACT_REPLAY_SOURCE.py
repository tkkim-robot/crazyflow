...
class ContactReplayConfig:
    ...
    obstacle_drive_mass_kg: float = 1000.0
...
class ObstacleMotion:
    ...
    def sample_velocity(self, time_seconds: np.ndarray | float) -> np.ndarray:
        """Derivative of the same center interpolant; right derivative at interior knots."""
        query = np.asarray(time_seconds, dtype=float)
        self.sample(query)  # Apply the identical finite/time-support contract.
        segment = np.clip(
            np.searchsorted(self.time_seconds, query, side="right") - 1,
            0,
            len(self.time_seconds) - 2,
        )
        slopes = np.diff(self.centers, axis=0) / np.diff(self.time_seconds)[:, None, None]
...
def build_contact_model(
    ...
    for index, radius in enumerate(obstacles.radii):
        # A mocap pose has zero solver velocity, even when its position is changed each step.
        # Prescribed free-joint qpos AND qvel convey the moving surface's normal momentum.
        obstacle = ET.SubElement(world, "body", name=f"obstacle_{index}", gravcomp="1")
        ET.SubElement(obstacle, "freejoint", name=f"obstacle_free_{index}", align="false")
        drive_inertia = 0.4 * config.obstacle_drive_mass_kg * float(radius) ** 2
        ET.SubElement(
            obstacle,
            "inertial",
            pos="0 0 0",
            mass=str(config.obstacle_drive_mass_kg),
            diaginertia=_numbers([drive_inertia] * 3),
        )
        ET.SubElement(
            obstacle,
            "geom",
...
def run_contact_replay(
    ...
    *,
    ...
    velocities = obstacles.sample_velocity(times)
    wind = (
    ...
    obstacle_addresses = [
        (
            int(model.joint(f"obstacle_free_{i}").qposadr[0]),
            int(model.joint(f"obstacle_free_{i}").dofadr[0]),
        )
    ...
    maximum_drive_position_correction = 0.0
    maximum_drive_velocity_correction = 0.0
    for index, when in enumerate(times):
        for obstacle_index, (qpos_address, dof_address) in enumerate(obstacle_addresses):
            if index:
                maximum_drive_position_correction = max(
                    maximum_drive_position_correction,
                    float(
                        np.linalg.norm(
                            data.qpos[qpos_address : qpos_address + 3]
                            - centers[index, obstacle_index]
                        )
                    ),
                )
                maximum_drive_velocity_correction = max(
                    maximum_drive_velocity_correction,
                    float(
                        np.linalg.norm(
                            data.qvel[dof_address : dof_address + 3]
                            - velocities[index, obstacle_index]
                        )
                    ),
                )
            data.qpos[qpos_address : qpos_address + 3] = centers[index, obstacle_index]
            data.qpos[qpos_address + 3 : qpos_address + 7] = [1, 0, 0, 0]
            data.qvel[dof_address : dof_address + 3] = velocities[index, obstacle_index]
            data.qvel[dof_address + 3 : dof_address + 6] = 0
        data.xfrc_applied[:] = 0
    ...
    metadata = {
        ...
        "model_semantics": (
            ...
            "prescribed wind and obstacle positions may vary. Obstacle free-joint poses AND "
            "world velocities are imposed from the absolute-time trajectory at every step. "
            "A large finite drive mass approximates an externally driven environment; the "
            "solver sees moving surfaces, but obstacle impact response is reset each step. "
            "This external trajectory drive can supply work and is not a closed isolated system."
        ),
        "obstacle_drive": {
            "mass_kg_per_obstacle": config.obstacle_drive_mass_kg,
            "drone_to_obstacle_mass_ratio": body.mass / config.obstacle_drive_mass_kg,
            "velocity_rule": "piecewise-linear center derivative; right derivative at knots",
            "maximum_step_position_correction_m": maximum_drive_position_correction,
            "maximum_step_velocity_correction_m_s": maximum_drive_velocity_correction,
        },
        "collider_semantics": (
        ...
        "integration": (
            "MuJoCo implicitfast, explicit 1-step mj_step calls; drone qpos/qvel assigned "
    ...
    return ContactReplay(
        times, states, centers, velocities, contacts, ground, forces, tuple(events), xml, metadata
    )
...
def save_contact_replay(result: ContactReplay, directory: str | Path) -> Path:
    ...
    np.savez_compressed(
        ...
        obstacle_velocities=result.obstacle_velocities,
    ...
    (directory / "CONTACT_REPLAY_SOURCE.py").write_bytes(Path(__file__).read_bytes())
    metadata = {
        **result.metadata,
