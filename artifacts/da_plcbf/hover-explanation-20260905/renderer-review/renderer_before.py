    ...
    for x, image, method, label, side in zip(
        ...
        if method.waypoint_index is not None:
            axis.text(
                0.97,
                0.972,
                f"Waypoint {method.waypoint_index[method_index] + 1}",
                color="#a7ffb8",
                fontsize=10 * scale,
                va="top",
                ha="right",
        ...
        if (
            _payload_is_attached(trace, method_index)
            and trace.time_seconds[method_index] < trace.payload_attachment_time_seconds + 3.0
        ):
            width = 0.105
            _draw_payload_detail(
                figure,
                (x + 0.365, 0.098, width, width * config.width / config.height),
                trace,
                method,
                method_index,
                label_size=6.7 * scale,
            )
