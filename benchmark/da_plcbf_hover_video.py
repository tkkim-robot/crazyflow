"""Render a full hover–avoid–return triplet with explicit contact-only suffixes."""
...
SOURCE = BASE / "selected-replay-v1"
...
def contact_samples(
    ...
    support = np.arange(round(DURATION * 1000) + 2) * 0.001
    ...
    config = ContactReplayConfig(duration_seconds=DURATION - trigger.time_seconds)
    ...
    try:
        with (output / "frame_audit.jsonl").open("x") as audit:
            for index, sample in enumerate(samples):
                ...
                draw.rounded_rectangle((20, 82, 600, 116), radius=5, fill="#102936")
                draw.text(
                    (30, 90),
                    f"Hover / avoid / return · {wind_text}",
                    font=_font(15),
                    fill="#9fe8fa",
                )
                home = np.asarray(episode.summary["hover_return"]["home_position_m"])
                error = float(np.linalg.norm(sample.state[:3] - home))
                speed = float(np.linalg.norm(sample.state[7:10]))
                windows = episode.summary["hover_return"]["return_windows"]
                returned = sum(
                    w["returned"] and w["dwell_completed_at_seconds"] <= TIMES[index]
                    for w in windows
                )
                draw.rectangle((440, 838, 613, 859), fill="#071018")
                draw.text(
                    (446, 842), f"Returns {returned}/{len(windows)}", font=_font(13), fill="#aabac5"
                )
                if not contact_mask[index]:
                    draw.rounded_rectangle((20, 164, 600, 199), radius=5, fill="#102936")
                    draw.text(
                        (30, 174),
                        f"Home distance {error:.2f} m · speed {speed:.2f} m/s",
                        font=_font(15),
                        fill="#d9e4eb",
                    )
                ...
                row.update(
                    {
                        ...
                        "home_distance_m": error,
                        "speed_mps": speed,
                        "completed_return_windows": returned,
                        "method": method,
...
def combine() -> None:
    ...
    try:
        for index in range(len(TIMES)):
            ...
            if index in (0, 40, 100, 110, 112, 120, 160, 250, 320, 400, 440, 500, 560, 640):
                frame.save(OUTPUT / f"frame-{index:06d}.png")
        for reader in readers:
...
if __name__ == "__main__":
    ...
    DURATION = float(
        json.loads((SOURCE / "scene-plan.json").read_text())["scene"]["world"]["config"][
            "duration_seconds"
        ]
    )
    TIMES = np.arange(round(DURATION * 20) + 1) / 20
