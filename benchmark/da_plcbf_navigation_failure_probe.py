...
def main() -> None:
    ...
    for prefix in (
        ...
        args.run / "snapshots/nominal_reference",
    ):
    ...
    provenance = {
        ...
        "execution_environment": {
            ...
            for name in ("JAX_COMPILATION_CACHE_DIR", "XLA_PYTHON_CLIENT_PREALLOCATE", "XLA_FLAGS")
        },
    }
    ...
    parity = {
        ...
        "device": str(device),
        "leaf_differences": [
            {"path": str(path), "max_absolute_error": error}
            for (path, _), error in zip(
                jax.tree_util.tree_flatten_with_path(state)[0], differences, strict=True
            )
            if error
        ],
    }
    np.savez_compressed(
        args.output / "reconstruction_arrays.npz",
        **{
            f"leaf_{index:03d}_{label}": np.asarray(value)
            for index, (actual, expected) in enumerate(
                zip(jax.tree.leaves(state), jax.tree.leaves(verification.state), strict=True)
            )
            for label, value in (("reconstructed", actual), ("recorded", expected))
        },
    )
