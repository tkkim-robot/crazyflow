...
def render(run: Path, output: Path, contact_directory: Path | None, azimuth: float) -> Path:
    ...
    collision_case = report["methods"]["fixed"]["termination"] == "physical_collision"
    both_completed = all(row["termination"] == "completed" for row in report["methods"].values())
    if (
        not (collision_case or both_completed)
        or report["methods"]["adaptive"]["termination"] != "completed"
        or len(scene.additional_incoming) != 1
    ):
        raise ValueError("this caption protocol requires a confirmed two-mover outcome")
    if contact_directory and not collision_case:
        raise ValueError("a collider-safe comparison cannot include a contact continuation")
    splice = None
    ...
    both_completion = max(row["termination_time_seconds"] for row in report["methods"].values())
    captions = []
    for when in trace.time_seconds:
        if when < scene.wind_onset_seconds:
            ...
            if when >= both_completion:
                caption = (
                    "Both methods completed both waypoints · "
                    "the compensated fixed library also succeeds"
                )
            elif when >= scene.navigation_start_seconds:
                caption = (
                    "Both methods avoided the movers · both now follow the same waypoint route"
                )
            else:
                caption = (
                    "Wind stays on · both fallback mappings compensate the known dynamics · "
                    "both remain safe"
                )
        elif when < fixed_event:
            caption = (
    ...
    trace = replace(
        trace,
        title=(
            "Two incoming obstacles | model-compensated fixed and adaptive fallbacks"
            if both_completed
            else "Two incoming obstacles in persistent wind | fixed fallbacks vs online adaptation"
        ),
    ...
    config = renderer.ComparisonRenderConfig(
        ...
        comparison_note=(
            ...
            + (
                ...
                if contact_directory
                else "matched model-compensated fallbacks; both methods complete safely"
                if both_completed
...
def main() -> None:
    ...
    parser.add_argument("--camera-azimuth", type=float, default=225)
